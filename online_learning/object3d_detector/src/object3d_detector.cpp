...
// Added Libraries for GPIO communication
...
public:
  ...
  void extractClusterSafetyBox(pcl::PointCloud<pcl::PointXYZI>::Ptr pc);  
...
Object3dDetector::Object3dDetector() {
  ...
  point_cloud_sub_ = node_handle_.subscribe<sensor_msgs::PointCloud2>("/os_cloud_node/points", 1, &Object3dDetector::pointCloudCallback, this);
  ...
  private_nh.param<std::string>("frame_id", frame_id_, "os_sensor");
  private_nh.param<float>("z_limit_min", z_limit_min_, -0.7); // -0.8
  private_nh.param<float>("z_limit_max", z_limit_max_, 0.3); // 1.2
  private_nh.param<int>("cluster_size_min", cluster_size_min_, 5);
  private_nh.param<int>("cluster_size_max", cluster_size_max_, 30000);
  private_nh.param<float>("human_probability", human_probability_, 0.85);
...
void Object3dDetector::pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& ros_pc2) {
  ...
  //extractClusterSafetyBox(pcl_pc);
  visualize();
  classify();
...
int zone_[nested_regions_] = {1,1,1,3,3,3,3,2,3,3,3,3,3,3}; // for more details, see our IROS'17 paper.
void Object3dDetector::extractCluster(pcl::PointCloud<pcl::PointXYZI>::Ptr pc) {
  ...
  // Print Out Information when Obstacle detected (Any points within that zone)
  // Before cluster detection and bearing information
  if(indices_array[0].size() != 0 ) {
    //std::cout << "Time: "<< clock() << "--> Obstacle detected in Zone 1 --> Points Count:" << indices_array[0].size() << std::endl;
  }
  ...
  for(int i = 0; i < nested_regions_; i++) {
    ...
    if(indices_array[i].size() > cluster_size_min_) {
      ...
      if(i == 0 ) {
        //std::cout << " Zone 1 --> Number of clusters detected:" << cluster_indices.size() << std::endl;

        // GPIO Message Test
        // https://github.com/pjueon/JetsonGPIO

        // Via bashscript
        //system("/home/rethnaraj/git/xyz/catkin_ws/src/online_learning/object3d_detector/src/gpio.sh");
        
        // via ROS publisher subscriber
        // http://wiki.ros.org/ROS/Tutorials/WritingPublisherSubscriber%28c%2B%2B%29
        ros::NodeHandle n;
        ros::Publisher chatter_pub = n.advertise<std_msgs::String>("chatter", 1000);        
        std_msgs::String msg;
        std::stringstream ss;
        ss << "Zone 1 --> Number of obstacle-clusters detected:" << cluster_indices.size();
        msg.data = ss.str();
...
const int nested_zones_ = 2;
float zone_x_min[nested_zones_] = {-0.70,-3};
float zone_x_max[nested_zones_] = {0.3,3};
float zone_y_min[nested_zones_] = {-0.3,-1};
float zone_y_max[nested_zones_] = {0.3,1};
void Object3dDetector::extractClusterSafetyBox(pcl::PointCloud<pcl::PointXYZI>::Ptr pc) {
  features_.clear();
  ...
  // Detect Point Clouds in Nested Safety Box
  boost::array<std::vector<int>, nested_zones_> indices_array;
  for(int i = 0; i < pc_indices->size(); i++) {
    for(int j = 0; j < nested_zones_; j++) {    
      //if(zone_x_min[nested_zones_] < pc->points[(*pc_indices)[i]].x) {
        //if(zone_y_min[nested_zones_] < pc->points[(*pc_indices)[i]].y) { 
        //if(pc->points[(*pc_indices)[i]].x < zone_x_min[nested_zones_]) {
        //if(pc->points[(*pc_indices)[i]].y < zone_y_min[nested_zones_]) {          
        if((zone_x_min[j] < pc->points[(*pc_indices)[i]].x) && (pc->points[(*pc_indices)[i]].x < zone_x_max[j])) {
          if((zone_y_min[j] < pc->points[(*pc_indices)[i]].y) && (pc->points[(*pc_indices)[i]].y < zone_y_max[j])) {       
          //std::cout << "Entered Safety Box" << std::endl;
          indices_array[j].push_back((*pc_indices)[i]);
          break;        
        }
      }
  ...
  // Print Out Information when Obstacle detected (Any points within that zone)
  if(indices_array[1].size() != 0 ) {
    //std::cout << "Time: "<< clock() << "--> Obstacle detected in Zone 1 --> Points Count:" << indices_array[0].size() << std::endl;
  }

  float tolerance = 0.0;
  // Starts with Zone 1 (i=1) and Ignore Zone 0 (Robot's dimension)
  for(int i = 1; i < nested_zones_; i++) {
    tolerance += 0.1;
    if(indices_array[i].size() > cluster_size_min_) {
      boost::shared_ptr<std::vector<int> > indices_array_ptr(new std::vector<int>(indices_array[i]));
      pcl::search::KdTree<pcl::PointXYZI>::Ptr tree(new pcl::search::KdTree<pcl::PointXYZI>);
      ...
      if(i == 1) {
        ...
        // Via bashscript
        //system("/home/raj/git/xyz/catkin_ws/src/online_learning/object3d_detector/src/gpio.sh");
...
/*    marker.type = visualization_msgs::Marker::LINE_LIST;
    ...
    marker.color.b = 1.0f;
    marker.color.a = 0.3;
    marker.pose.position.x = (it->min[0]+it->max[0])/2;
    marker.pose.position.y = (it->min[1]+it->max[1])/2;
    marker.pose.position.z = (it->min[2]+it->max[2])/2;
...
void Object3dDetector::visualize() {
  visualization_msgs::MarkerArray marker_array;
  geometry_msgs::PoseArray pose_array;
